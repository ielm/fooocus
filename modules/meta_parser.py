...
def load_parameter_button_click(raw_metadata: dict | str, is_generating: bool):
    ...
    get_number(
        "adaptive_cfg", "CFG Mimicking from TSNR", loaded_parameter_dict, results
    )
...
class A1111MetadataParser(MetadataParser):
    ...
    def to_json(self, metadata: str) -> dict:
        ...
        *lines, lastline = metadata.strip().split("\n")
        ...
        for key in ["base_model", "refiner_model", "vae"]:
            if key in data:
                if key == "vae":
                    self.add_extension_to_filename(
                        data, modules.config.vae_filenames, "vae"
                    )
                else:
                    self.add_extension_to_filename(
                        data, modules.config.model_filenames, key
                    )
        ...
        if "lora_weights" in data and data["lora_weights"] != "":
            ...
            "lora_hashes" in data
            and data["lora_hashes"] != ""
            and data["lora_hashes"].split(", ")[0].count(":") == 2
        ):
    ...
    def to_string(self, metadata: dict) -> str:
        ...
        if len(self.loras) > 0:
            ...
            generation_params[self.fooocus_to_a1111["lora_weights"]] = (
                lora_weights_string
            )
        ...
        for key, value in metadata.items():
            ...
            if key in ["base_model", "refiner_model"]:
                metadata[key] = self.replace_value_with_filename(
                    key, value, modules.config.model_filenames
                )
            elif key.startswith("lora_combined_"):
                metadata[key] = self.replace_value_with_filename(
                    key, value, modules.config.lora_filenames
                )
            elif key == "vae":
                metadata[key] = self.replace_value_with_filename(
                    key, value, modules.config.vae_filenames
                )
            else:
                continue
