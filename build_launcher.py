...
is_win32_standalone_build = os.path.exists(python_embeded_path) and os.path.isdir(
    python_embeded_path
)
...
def build_launcher():
    ...
    for preset in presets:
        win32_cmd_preset = win32_cmd.replace(
            "{cmds}", "" if preset is None else f"--preset {preset}"
        )
        bat_path = os.path.join(
            win32_root, "run.bat" if preset is None else f"run_{preset}.bat"
        )
