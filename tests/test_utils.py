...
class TestUtils(unittest.TestCase):
    def test_can_parse_tokens_with_lora(self):
        test_cases = [
            {
                "input": (
                    "some prompt, very cool, <lora:hey-lora:0.4>,  cool   <lora:you-lora:0.2>",
                    [],
                    5,
                    True,
                ),
                "output": (
                    [("hey-lora.safetensors", 0.4), ("you-lora.safetensors", 0.2)],
            ...
            {
                "input": (
                    "some prompt, very cool, <lora:hey-lora:0.4><lora:you-lora:0.2>",
                    [],
                    3,
                    True,
                ),
            ...
            # test deduplication, also selected loras are never overridden with loras in prompt
            {
                ...
                "output": (
                    [("you-lora.safetensors", 0.3), ("hey-lora.safetensors", 0.4)],
                    "some prompt, very cool",
                ),
            },
            {
        ...
        for test in test_cases:
            ...
            actual = util.parse_lora_references_from_prompt(
                prompt, loras, loras_limit=loras_limit, skip_file_check=skip_file_check
            )
    ...
    def test_can_parse_tokens_and_strip_performance_lora(self):
        ...
        test_cases = [
            {
                "input": (
                    "some prompt, <lora:hey-lora:0.4>",
                    [],
                    5,
                    True,
                    modules.flags.Performance.QUALITY,
                ),
                "output": ([("hey-lora.safetensors", 0.4)], "some prompt"),
            },
            {
                "input": (
                    "some prompt, <lora:hey-lora:0.4>",
                    ...
                    5,
                    True,
                    modules.flags.Performance.SPEED,
                ),
                "output": ([("hey-lora.safetensors", 0.4)], "some prompt"),
            },
            {
                "input": (
                    "some prompt, <lora:sdxl_lcm_lora:1>, <lora:hey-lora:0.4>",
                    [],
                    5,
                    True,
                    modules.flags.Performance.EXTREME_SPEED,
                ),
                "output": ([("hey-lora.safetensors", 0.4)], "some prompt"),
            },
            {
                "input": (
                    "some prompt, <lora:sdxl_lightning_4step_lora:1>, <lora:hey-lora:0.4>",
                    [],
                    5,
                    True,
                    modules.flags.Performance.LIGHTNING,
                ),
                "output": ([("hey-lora.safetensors", 0.4)], "some prompt"),
            },
            {
                "input": (
                    "some prompt, <lora:sdxl_hyper_sd_4step_lora:1>, <lora:hey-lora:0.4>",
                    [],
                    5,
                    True,
                    modules.flags.Performance.HYPER_SD,
                ),
                "output": ([("hey-lora.safetensors", 0.4)], "some prompt"),
            },
        ]
        ...
        for test in test_cases:
            ...
            lora_filenames = modules.util.remove_performance_lora(
                lora_filenames, performance
            )
            expected = test["output"]
            actual = util.parse_lora_references_from_prompt(
                prompt, loras, loras_limit=loras_limit, lora_filenames=lora_filenames
            )
            self.assertEqual(expected, actual)
