...
def sample_tcd(
    ...
    timesteps_s = (
        torch.floor((1 - eta) * model_sampling.timestep(sigmas))
        .to(dtype=torch.long)
        .detach()
        .cpu()
    )
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        denoised = (
            alpha_prod_s[i + 1].sqrt() * denoised + beta_prod_s[i + 1].sqrt() * eps
        )
        ...
        if eta > 0 and sigmas[i + 1] > 0:
            ...
            x = (
                x / alpha_prod_s[i + 1].sqrt()
                + noise * (sigmas[i + 1] ** 2 + 1 - 1 / alpha_prod_s[i + 1]).sqrt()
            )
        else:
